...
dt <- read.csv("split_result.csv")
dt$pp <- as.logical(dt$pp)

seldt <- function(classifier, pp) {
    dt[dt$classifier == classifier & dt$pp == T, !names(dt) %in% c("pp", "classifier")]
}
@
...
\begin{itemize}
    ...
    as tokenization is concerned, we applied fancy token-level analysis (lemmatizing)
    in the tokenization step and normalized the token occurence counts (l1 norm).
    Stop words were also removed.
    \item Training: We trained a \texttt{SGDClassifier} and a \texttt{MultinominalNB}
    classifier for $\{ 2^n | n = 10, \ldots, 22\}$ training samples with minimal
    ...
    list), we iteratively take out a fixed number of \texttt{BATCH\_SIZE} instances
...
\section{Results}
\subsection{SGD}
\subsubsection{With minimal pre-processing}
<<results="asis">>=
print(xtable(sel("SGDClassifier", F)), table.placement="H", include.rownames=F)
@
\subsubsection{With full pre-processing}
<<results="asis">>=
print(xtable(sel("SGDClassifier", T)), table.placement="H", include.rownames=F)
@
\subsection{Ridge}
\subsubsection{With minimal pre-processing}
<<results="asis">>=
print(xtable(sel("MultinominalNB", F)), table.placement="H", include.rownames=F)
@
\subsubsection{With full pre-processing}
<<results="asis">>=
print(xtable(sel("MultinominalNB", T)), table.placement="H", include.rownames=F)
@
